...
public class RealCamera : MonoBehaviour {
	...
	void LateUpdate() {
		...
		HorizontalAxis = Input.GetAxis("Mouse X") * 15;
		VerticalAxis = Input.GetAxis("Mouse Y") * 15;
	...
	void occludeRay(ref Vector3 targetFollow) {
		...
		if (Physics.Linecast(targetFollow, camMask, out wallHit, CamOcclusion)) {
			...
			// if (camPosition.y < target.transform.position.y + 5){
            //     camPosition = new Vector3(camPosition.x, target.transform.position.y + 5, camPosition.z);
            // }
		}
		#endregion
	}
